...
/* Variables locals */
   int ib, j, k, l, iret;
   double tt, bet, d, wksp[15*n], *r=wksp, *b=wksp+13*n,
    *f=wksp+14*n;
/* Bucle en el pas */
   do {
   /* Fem RK7 -> b[] i RK8 -> f[]
   ...
   if (iret) {
    ...
    return iret;
      ...
      d/=n; /* No es la norma sub-1, sino el promig dels error   */
    /*    comesos a cada coordenada. Ã‰s un factor de la  */
                /*    norma-, S'assembla a la norma-inf si coords    */
                /*    semblants i Ã©s mÃ©s rÃ pid                       */
   /* Pleguem si O.K. o pas minim. */
      if (d<tol || fabs(*h)<=hmin) break;
...
/* Fem correccio de pas */
   if (d<tol) { double tmp=tol/256; if (d<tmp) d=tmp; }
        /* Si l'error ha estat molt petit   */
        /*    no volem que el nou pas es dispari. */
   (*h)*=.9*pow(tol/d,.125);  /* Correccio Fehlberg (Stoer 2a ed (7.2.5.16), */
